...
def test_algorithm(test: ProblemInstance, expected_result: ProblemInstance, algorithm: Callable[[list[float], list[int], list[float]], None], name: str):
	...
	print("Assignments: ", test_copy.assignments)
	print("Free Space: ", test_copy.free_space, "\n")
...
def my_zip_tests():
	print('testing My ZipTree')
	
	tree = requirements.ZipZipTree(10)			# creating a tree with capacity 10. just to start ahfgrdhkrdl
	tree.insert(4, 'a', requirements.Rank(0, 9))
	
	tree.insert(5, 'b', requirements.Rank(0, 9))

	tree.insert(2, 'c', requirements.Rank(1,12))
	
	tree.insert(1, 'd', requirements.Rank(1,5))
	...
	tree.print_tree(tree.root)
	tree.remove(2)
	print("After removal of (2, 'c')")
	tree.print_tree(tree.root)
	
	tree2 = requirements.ZipZipTree(10)			# creating a tree with capacity 10. just to start ahfgrdhkrdl
	tree2.insert(4, 'a', requirements.Rank(2,1))
	
	print("tree2 tests:")
	
	tree2.insert(5, 'b', requirements.Rank(2,2))

	tree2.insert(2, 'c', requirements.Rank(1,8))
	tree2.print_tree(tree2.root)

	tree2.insert(1, 'd', requirements.Rank(0,12))
	tree2.print_tree(tree2.root)
